...
export default async function GroupDashboardPage(props: { searchParams: Promise<{ month?: string; category?: string }> }) {
  ...
  const expenses = await getGroupExpenses(currentMonth, searchParams.category);
  const totalAmount = expenses.reduce((sum: number, exp: { amount: number }) => sum + exp.amount, 0);

  // Settlement matches the current filter view
      ...
      <div className="dashboard-main">
         <div className="dashboard-header">
           <h2 style={{ fontSize: '1.2rem', fontWeight: 600 }}>
            チームの支出一覧
             {/* {currentMonth.split('-')[0]}年 {parseInt(currentMonth.split('-')[1])}月 チームの支出一覧 */}
           </h2>
           <FilterBar />
         </div>
